...
void Put_a_Loop_in_Input(struct node *,int);
...
void Put_a_Loop_in_Input(struct node *root,int k)
{
	int i=0;
...
char *Is_Loop(char *input,struct node *root)
{
	...
	if(root!=NULL&&root->next!=0)
	{
		slow=root;
		fast=root->next->next;
		while(root->next!=0&&fast!=0&&slow!=0)
		{
			if(slow==fast)
				return "Yes";
			slow=slow->next;
